  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="max-h-80 overflow-y-auto px-2 py-2">
          {plugins.length === 0 && (
            <div className="px-3 py-10 text-center text-sm" style={{ color: 'var(--text-dim)' }}>
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" className="mx-auto mb-3" style={{ color: 'var(--text-dim)', opacity: 0.4 }}>
                <rect x="3" y="3" width="18" height="18" rx="2" /><line x1="12" y1="8" x2="12" y2="16" /><line x1="8" y1="12" x2="16" y2="12" />
              </svg>
              {t('plugin.noPlugins')}<br/>{t('plugin.help')}
            </div>
          )}
          {plugins.map(p => (
            <div
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] shrink-0" style={{ color: 'var(--text-dim)' }}>v{p.version}</span>
                </div>
                {p.description && (
                  <div className="text-xs mt-0.5 truncate" style={{ color: 'var(--text-dim)' }}>{p.description}</div>
                )}
              </div>
              {/* Toggle switch */}
              <button
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-5 py-3 border-t" style={{ borderColor: 'var(--border)', background: 'var(--bg-surface)' }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onMouseEnter={e => { if (!refreshing) e.currentTarget.style.background = 'var(--accent-hover)' }}
            ...
            >
              {refreshing ? (
                ...
                <span className="flex items-center gap-1">
                  <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round">
                    <polyline points="14,8 12,10 10,8" /><path d="M12 10a6 6 0 1 1-2-5" />
                  </svg>
                  {t('plugin.refresh')}
                </span>
              )}
